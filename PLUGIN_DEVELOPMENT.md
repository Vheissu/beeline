...
//
// SECURITY: every broadcastCustomJson call is gated by an explicit user
// confirmation. Before signing, Beeline renders the real operation it is about
// to sign (your plugin name, the account, the signing authority, the custom_json
// id, and the JSON payload) and asks the user to approve. If the user declines,
// the call rejects with "Plugin broadcast declined by user" — handle that case
// gracefully. The confirmation text is generated by Beeline, not your plugin.
```
